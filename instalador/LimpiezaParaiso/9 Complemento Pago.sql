...
P.LUGAREXPEDICION,(SELECT vrs.RFC FROM [DB_COMPIDTFAC].dbo.VARSIS vrs) AS rfc,
(SELECT cr.C_REGIMENFISCAL from [DB_COMPIDTFAC].dbo.CAT_REGIMENFISCAL cr WHERE cr.DESCRIPCION = p.REGIMEN) AS rclave,
 P.REGIMEN,(SELECT vrs.contribuyente FROM [DB_COMPIDTFAC].dbo.VARSIS vrs) AS nombre,
 ...
 (SELECT cf.C_FORMAPAGO from [DB_COMPIDTFAC].dbo.CAT_FORMAPAGO cf WHERE cf.DESCRIPCION = rp.FORMADEPAGOP) AS clave,
 ...
 FROM [DB_COMPIDTFAC].dbo.PAGOS33 P INNER JOIN [DB_COMPIDTFAC].dbo.RECEPCION_PAGOS RP ON(P.Serie = RP.SERIE AND P.Folio = RP.FOLIO)
 LEFT JOIN [DB_COMPIDTFAC].dbo.DATOSELECTRONICA d ON(D.Serie = RP.SERIE AND D.Folio = RP.FOLIO)
