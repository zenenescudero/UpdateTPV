...
SELECT row_number() over(order by n.fecha) as Id, 0 as ticket, 1 sucursal,n.CLIENTE,1 hist,'' ver,n.NCRSERIE,d.FOLIO,
 ...
 'MXN' moneda,1 cambio,
  n.TOTAL,
 (SELECT ct.C_TIPODECOMPROBANTE FROM DB_MUEBLERIA.dbo.CAT_TIPODECOMPROBANTE ct WHERE ct.DESCRIPCION = n.TIPODECOMPROBANTE) AS tipo
 , n.TIPODECOMPROBANTE,(SELECT cm.C_METODOPAGO from dbo.CAT_METODOPAGO cm WHERE cm.C_METODOPAGO = n.METODOPAGO) mclave,
  n.METODOPAGO, (SELECT v.CP from DB_MUEBLERIA.dbo.VARSIS v) expedicion,
  (SELECT v.RFC from DB_MUEBLERIA.dbo.VARSIS v) emirsorrfc,
  (SELECT cr.C_REGIMENFISCAL from dbo.CAT_REGIMENFISCAL cr WHERE cr.DESCRIPCION = n.REGIMEN) regimen,
   n.REGIMEN ,(SELECT v.contribuyente from DB_MUEBLERIA.dbo.VARSIS v) emirsornombre,
   ...
   0 ussa,n.QUIEN, n.CUANDO, n.DONDE,0,1,1,1   
     FROM DB_MUEBLERIA.dbo.NCRDEVOLUCION n LEFT JOIN
DB_MUEBLERIA.dbo.DATOSELECTRONICA d on(n.NCRSERIE = d.SERIE AND n.FOLIO = d.FOLIO) 
